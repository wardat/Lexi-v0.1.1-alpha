...
//import com.jmonkey.core.sys.ThreadPool;
...
public final class TEXTEditor extends Editor implements MouseListener, KeyListener {
	...
	public void append(File file) throws IOException {
		EditorActionManager.threads(new FRThread(file) {
										...
										try {
												...
												_EDITOR.getEditorKit().read(br, _EDITOR.getDocument(), _EDITOR.getDocument().getLength());
												setChanged(true);
										} catch(BadLocationException ble0) {
												//Code.failed(ble0); ///throw new IOException(ble0.getMessage());
										} catch(FileNotFoundException fnfe0) {
												//Code.failed(fnfe0); // throw new IOException(fnfe0.getMessage());
										} catch(IOException ioe0) {
												//Code.failed(ioe0); //throw ioe0;
										} finally {
											if(br != null) {
												try {
														br.close();
	...
	*/
	...
	public void hasBeenActivated(Editor editor) {
		if(editor == this) {
			EditorActionManager.enableFormatActions(false);
	...
	public void insert(File file, int position) throws IOException {
		EditorActionManager.threads(new FRThread(file, position) {
										...
										try {
												br = new BufferedReader(new FileReader(this.file));
												_EDITOR.getEditorKit().read(br, _EDITOR.getDocument(), this.position);
												setChanged(true);
										} catch(BadLocationException ble0) {
												//Code.failed(ble0); // throw new IOException(ble0.getMessage());
										} catch(FileNotFoundException fnfe0) {
	...
	*/
	...
	public void read(File file) throws IOException {
		EditorActionManager.threads(new FRThread(file) {
										...
										try {
												br = new BufferedReader(new FileReader(this.file));
												_EDITOR.getEditorKit().read(br, _EDITOR.getDocument(), 0);
												setChanged(false);
										} catch(BadLocationException ble0) {
												//Code.failed(ble0);
	...
	public void write(File file) throws IOException {
		EditorActionManager.threads(new FWThread(file) {
										...
										try {
												...
												_EDITOR.getEditorKit().write(bw, _EDITOR.getDocument(), 0, _EDITOR.getDocument().getLength());
												setChanged(false);
										} catch(BadLocationException ble0) {
												//Code.failed(ble0);
												//throw new IOException(ble0.getMessage());
